...
contract MerkleAirdropTest is ZkSyncChainChecker, Test {
    ...
    function testUsersCanClaim() public {
        ...
        // gasPayer calls claim using the singned message
        vm.prank(gasPayer);
    ...
    function testGetMerkleRoot() public view {
        bytes32 expectedRoot = airdrop.getMerkleRoot();
        assertEq(ROOT, expectedRoot);
    }

    function testGetAirdropToken() public view {
        address expectedAirdropToken = address(airdrop.getAirdropToken());
        assertEq(address(token), expectedAirdropToken);
    }
}
